...
export function Song({ id, title, url, cover, played }) {
  return (
    <div className="text-white">
      ...
      <Link href={`/song/${id}`}>
        <p className=" truncate"><a href={url} target="_blank">{title}</a></p>
      </Link>
    </div>
  )
}
...
function Songs() {
  ...
  return (
    <main className="grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 grid gap-4 p-4 bg-gray-700  m-10 rounded-lg border-b-4 border-pink-500">
